  ...
  target_list_file.each_line do |file|
    ...
    File.open(file) do |f|
      ...
      f.each_line do |line|
        ...
        # $this->params系チェック
        elsif /(this->(here|header|webroot|data[^a-zA-Z]|params|action[^a-zA-Z]|params\['form'\]))/.match(line) ||
          /(cakeError)/.match(line) ||
          /(this->Auth->allow\((array\('\*'\)\)|\('\*'\)))/.match(line) then
          update_hash(results,file,$1,line,line_num)
        # $this->RequestHandler系チェック
        elsif /this->RequestHandler->(is|setContent)/.match(line) then
          update_hash(results,file,$1,line,line_num)
        # Cacheキーチェック
...
system("php #{root_path}/.circleci/send_message.php $(cat #{cakephp28_log})")
